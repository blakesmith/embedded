...
int AlsaOutput::Start() {
    ...
    if (rc < 0) {
        keebird_log_verbose("ALSA", "Failed to prepare device for use: %s\n", snd_strerror(rc));
        return rc;
    }
    ...
    keebird_log_verbose("ALSA", "period size: %lu\n", frames);
...
void AlsaOutput::Stop() {
    ...
    keebird_log_verbose("ALSA", "Closing playback device: %s\n", device_name_);
    if (pcm_device_ != nullptr) {
        snd_pcm_drain(pcm_device_);
        rc = snd_pcm_close(pcm_device_);
        if (rc < 0) {
            keebird_log_verbose("ALSA", "Warning, failed to close device: %s\n", snd_strerror(rc));
        }
    }
}
...
int AlsaOutput::Write(const int16_t* pcm_buf, size_t size) {
    ...
    if (written < 0) {
        keebird_log_verbose("ALSA", "Failed to write, %s\n", snd_strerror(written));
        return written;
    }
